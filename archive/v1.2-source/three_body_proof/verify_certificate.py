...
def verify(c):
    if c.get('schema') != 'three-body-interval-certificate-1' or c.get('bits') != BITS:
    ...
    T = Fraction(inp['T'])
    order = inp['order']
    if not 0 < T <= 2 or type(order) is not int or not 4 <= order <= 16: raise ValueError('invalid integration parameters')
    steps = c['steps']
