...
out=json.loads(subprocess.run(['node','-e',js,str(BASE/'validated.js')],input=json.dumps({'cases':cases,'parse_values':parse_values}),capture_output=True,text=True,check=True,timeout=20).stdout)
checks=0
...
for text,interval in zip(parse_values,out['parsed']):
    ...
    checks+=1
base=json.loads((BASE/'asymmetric_Tp1.json').read_text())
mutations=[]
...
j=json.loads(subprocess.run(['node','-e',js,str(BASE/'validated.js')],input=json.dumps([c for _,c in mutations]),capture_output=True,text=True,check=True,timeout=20).stdout)
assert all(j)
...
(BASE/'exact_test_results.json').write_text(json.dumps(result,indent=2));print(json.dumps(result,indent=2))
